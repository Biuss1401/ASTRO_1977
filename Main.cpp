...
bool Load_Assets()
{
	...
	if (!Player_Spaceship.Load_Image("Assets/Spaceship.png", G_Renderer))
	{
		cout << "Image Error: " << endl;
		success = false;
	}
...
void Reset_Game()
{
	...
	while (Player_Spaceship.get_Bullets().size() > 0)
	{
		Player_Spaceship.remove_Bullets(i);
	}
...
int main(int argc, char* argv[])
{
	...
	while (game_Running)
	{
		...
		while (Playing)
		{
			...
			Player_Spaceship.Move(G_Renderer);
			...
			for (int i = 0; i < Player_Spaceship.get_Bullets().size(); i++)
			{
				...
				{
					if (Check_Collision(Player_Spaceship.get_Bullets()[i]->Get_Rect(), Big_Asteroids[big]->Get_Rect()))
					{
						Player_Spaceship.remove_Bullets(i);
						bullet_removed = true;
						Big_Asteroids[big]->Attacked(Player_Spaceship.get_Bullet_dmg());
				...
				{
					if (Check_Collision(Player_Spaceship.get_Bullets()[i]->Get_Rect(), Medium_Asteroids[med]->Get_Rect()))
					{
						Player_Spaceship.remove_Bullets(i);
						bullet_removed = true;
						Medium_Asteroids[med]->Attacked(Player_Spaceship.get_Bullet_dmg());
				...
				{
					if (Check_Collision(Player_Spaceship.get_Bullets()[i]->Get_Rect(), Small_Asteroids[small]->Get_Rect()))
					{
						Player_Spaceship.remove_Bullets(i);
						bullet_removed = true;
						Small_Asteroids[small]->Attacked(Player_Spaceship.get_Bullet_dmg());
			...
			for (int i = 0; i < Player_Spaceship.get_Bullets().size(); i++)
			{
				if (Check_Collision(Player_Spaceship.get_Bullets()[i]->Get_Rect(), Player_Spaceship.Get_Rect()))
				{
					Player_Spaceship.remove_Bullets(i);
					Player_Spaceship.Get_Damage(Player_Spaceship.get_Bullet_dmg());
					if (Player_Spaceship.get_current_HP() <= 0)
					{
						Playing = false;
						Death_menu = true;
					}
			...
			{
				if (Check_Collision(Player_Spaceship.Get_Rect(), Big_Asteroids[big]->Get_Rect()))
				{
					player_score += BIG_ASTEROID_SCORE;
					//spaceship take damage
					Player_Spaceship.Get_Damage(BIG_ASTEROID_DAMAGE);
			...
			{
				if (Check_Collision(Player_Spaceship.Get_Rect(), Medium_Asteroids[med]->Get_Rect()))
				{
					player_score += MEDIUM_ASTEROID_SCORE;
					Player_Spaceship.Get_Damage(MEDIUM_ASTEROID_DAMAGE);
			...
			for (int small = 0; small < Small_Asteroids.size(); small++)
			{
				...
				if (Check_Collision(Player_Spaceship.Get_Rect(), Small_Asteroids[small]->Get_Rect()))
				{
					player_score += SMALL_ASTEROID_SCORE;
					Player_Spaceship.Get_Damage(SMALL_ASTEROID_DAMAGE);
			...
			Player_Spaceship.Render(G_Renderer);
			SDL_RenderPresent(G_Renderer);
		}
